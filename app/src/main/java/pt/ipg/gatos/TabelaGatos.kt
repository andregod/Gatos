...
class TabelaGatos(db: SQLiteDatabase) : TabelaBD(db, NOME_TABELA) {
    ...
    override fun criar() {
        db.execSQL("CREATE TABLE $NOME_TABELA ($CHAVE_TABELA, $CAMPO_NOME TEXT NOT NULL, $CAMPO_COR TEXT, $CAMPO_IDADE INTEGER, $CAMPO_PESO FLOAT NOT NULL, $CAMPO_NOMEDONO TEXT, $CAMPO_MORADA TEXT, $CAMPO_PORTE TEXT , $CAMPO_DATA_NASC INTEGER ,$CAMPO_FK_RACA INTEGER NOT NULL, FOREIGN KEY ($CAMPO_FK_RACA) REFERENCES ${TabelaCategorias.NOME_TABELA}(${BaseColumns._ID}) ON DELETE RESTRICT)")
    }
    ...
    companion object {
        ...
        val CAMPOS = arrayOf(BaseColumns._ID, CAMPO_NOME, CAMPO_COR, CAMPO_DATA_NASC,CAMPO_IDADE,CAMPO_PESO, CAMPO_NOMEDONO,CAMPO_MORADA,CAMPO_PORTE ,CAMPO_FK_RACA)
